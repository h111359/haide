...
Prompt version: 20260922_095100Z
Generated (UTC): 2026-09-22 09:51:00Z
...
Saving input, questionnaire answers, or amendments creates drafts only. Receiving or uploading a requestor response stages an import only. Neither saving nor uploading starts execution or silently changes the scope of an active process.

Explicit Clarify, Analyze, Implement, Implement directly, or Resume actions submit the latest relevant saved input, reviewed answer drafts, and pending saved amendments. Raw or unreviewed requestor uploads are excluded from requirement adoption. Capture immutable submission revisions, source-form/import and amendment references, attachment identities and content fingerprints, action, timestamp, and owning request. Detect repeated submissions and commands idempotently. The explicitly labeled Import answers and clarify action performs the reviewed merge and this same Clarify submission contract; it is not an automatic consequence of uploading.
...
Use the same safe checkpoint, ownership, and one-writer rules as other actions. If a process is active, submit the revision for controlled adoption instead of starting a second writer. Preserve the requested Clarify action at that checkpoint: transition to requirements clarification and do not implicitly resume implementation merely because the submitted inputs were accepted. Existing scoped authorization remains recorded, while further implementation requires its explicit applicable action. Resumption can continue a supported native session or start a clearly identified new segment from persisted evidence; the user should not need to reconstruct the conversation. Persist progress so interruption after receiving answers, committing a submission, or updating interpretation can be reconciled without duplicate effects.

Provide an Amendments area within Current request > Input and clarification, available throughout the open request, with an Add amendment action. Each amendment has a stable ID, optional short title, plain-text/Markdown description of what changed or what new information is available, optional reason, source/attribution, creation and save timestamps, ordering, and links to affected requirements/questions or a superseded amendment when known. The framework user can change their mind, supply additional details, correct a previous statement, or withdraw an earlier proposed requirement. These are explicit request inputs; changing a form does not silently rewrite submitted history.

Keep amendment drafts and their histories inside the owning request, for example `amendments/<amendment-id>/draft.md` with immutable `revisions/<revision>.md` snapshots and a compact `amendments/catalog.yaml`. Define the exact schema and paths in conventions. Preserve every amendment revision explicitly saved through the framework and every submitted revision; unsaved keystrokes need not be archived. For direct edits in an external editor, capture the changed version at the next framework read/save/submission and retain the preceding captured version; do not claim that intermediate external saves never observed by the framework were recorded. Document the framework save operation for file-only users who need every revision captured. Show draft, saved-not-submitted, submitted, applied, superseded, withdrawn, and needs-clarification states with source/round references as applicable. Submitted corrections or withdrawals append a new revision/event rather than erasing previous content. Readable catalog/status views are derived from those records and the existing ledger; do not create a competing requirements definition.
...
1. **Upload requestor answers** accepts file selection and drag-and-drop for the generated `.txt` form; **Paste answers** supports responses returned as text. State supported types/size limits and give actionable encoding/format errors. Preserve the original received content before proposing changes, assign a receipt ID, and identify the target request/form. Uploading or pasting only stages the receipt; no agent runs and no active answer is overwritten.
...
The combined action must bind its review to the form, question, existing answer-draft, input, and amendment revisions actually displayed. A changed revision requires refreshing/reconciling the review; never overwrite intervening user edits or silently include newly changed requirements unseen by that review. Explain and resolve unsaved edits elsewhere before submitting. Persist the reviewed merge, immutable submission, and pending execution identity through a recoverable transaction, then launch or schedule the controlled worker. A launch failure leaves an inspectable saved submission that can resume; retrying cannot duplicate receipt application, amendment effects, or runs.

For an older returned form, show its original questions. Answers to unchanged identified question revisions can be proposed for review; answers to changed or withdrawn questions require explicit resolution. Conflicting currently saved/submitted answers must never be overwritten on import without review. A missing form reference or question ID offers explicit manual matching with provenance; do not guess by position. A form belonging to another or closed request is retained as an unassigned receipt for correction in the current intake context, with no mutation or reopening of that other request. Define the temporary unassigned intake under the existing `input/` directory; after explicit association, move/link the receipt into its owning request through a recoverable helper operation, preserving its ID and source. If no request is open, offer read-only inspection and guidance to start an allowed new request; do not create a request from an upload implicitly.

Text outside recognized answer fields is retained and displayed for review. New information, changed wishes, or additional requirements in comments can be explicitly linked to a proposed amendment or accepted as attributed requirement input; avoid duplicating the same change as both a new answer and a separate unlinked amendment. A requestor response cannot approve a plan, enable a skill, change human-owned instructions, expand execution permissions, or start implementation. Record the requestor as the attributed source and the framework user as reviewer/submitter.

Expose equivalent deterministic CLI/helper operations to export forms, receive files or pasted text, inspect/review mappings, save answer drafts, manage amendment revisions, and submit Clarify. File-only users can edit the authoritative Markdown answer drafts and amendment files directly, with the same snapshot/conflict rules. Preserve imported originals and their links when transferring answers manually. Import matching, parsing, revision checks, rendering, and audit bookkeeping belong to Python helpers; semantic explanation writing, requirement interpretation, and ambiguity resolution belong to the clarification skill and the user review.

Keep all drafts, exported snapshots, received originals, reviews, submissions, and resulting interpretation revisions linked in the request audit history. Unreviewed receipts remain distinct from active answers and are not silently loaded as instructions on the next run. Exports and reviewed blocker-resolution submissions stay available while clarification is waiting; they do not permit unrelated change work to proceed.
...
|---|---|---|
| `clarify-requirements` | Iteratively establish what must change and why: problem, users, outcomes, current versus required behavior, scope, constraints, acceptance criteria, contradictions, and unanswered questions. Apply reviewed framework-user/requestor answers and submitted amendments; explain questions for non-technical requestors. Use during Clarify or when a requirement gap is exposed. Read-only investigation is permitted; choosing an implementation is not. | Versioned canonical interpretation, explained internal/requestor questionnaires, reviewed answer/source links, round summaries, amendment dispositions, and explicit unresolved requirements. |
...
Use a persistent header showing product, active request, phase, execution status, and selected profile, with explicit empty/initializing states. Make the relevant next action prominent and keep other allowed actions discoverable. During clarification, distinguish waiting for the framework user, waiting for the requestor, unresolved conflicts, and ready for Analysis; offer the corresponding answer-import, amendment, Clarify, or Analyze action without automatically advancing stages. Use ordinary language on human-facing pages; IDs, hashes, and detailed technical evidence remain available on demand.

Keep Save draft, submit/run actions, Stop execution, and Close request visibly distinct. Saving forms and uploading requestor answers never run the agent. The reviewed Import answers and clarify action is an explicit combined submission/run command; its label and feedback must make that effect clear. Show pending same-request submissions separately from executing work. Explain unavailable actions with their specific cause, such as an open request, stale approval, unresolved blocker, missing profile, or failed required test. Do not silently queue a new request.
...
Provide one discoverable Python CLI, usable from the product root as `python .aih/engine/cli.py <command>`. Document the equivalent Python launcher usage on Windows and provide `--help` for every command, including the distinction between repeatable Clarify (what and why, applying reviewed answers and amendments), repeatable Analyze (how, including plan generation), and Implement (execute the authorized plan). Implement the named operations below; select and document exact arguments consistently.
...
- Clarify, analyze, approve, implement, implement directly, resume, stop execution, and close a request with an explicit outcome. Use `clarify` for every requirements-clarification iteration and `analyze` for every analysis iteration and automatic plan generation; do not require or expose a standalone `plan` command.
- Export/preview requestor questionnaires, receive plain-text answers, inspect and explicitly apply reviewed answer mappings, and optionally combine that reviewed application with Clarify submission. Create/save/list/compare amendment revisions and their submitted/applied/superseded states. Pure export, receipt intake, draft editing, and review operations do not invoke an agent; the combined command must explicitly opt into Clarify execution and reuse its contracts.
...
48. Exercise upload/drop and paste, review of matched answers, partial and unknown answers, duplicate IDs/receipts, missing references with manual matching, altered question text, stale forms, newer local answers, answers for withdrawn questions, wrong/closed-request forms, and free-text proposed amendments. Preserve originals, distinguish attributed requestor from actual submitter, prevent unreviewed answer adoption, and verify these inputs cannot grant plan approval or implementation authority.
49. Verify Save reviewed answers only saves drafts, while Import answers and clarify commits the reviewed merge and explicit submission. Simulate revision changes during review, repeated clicks, interruption between merge/submission/worker launch, and a launch failure. Recover without lost newer edits, duplicate effects, or a second writer; report saved/submitted/pending/running accurately.
50. Preserve amendment IDs, every framework-saved/submitted revision and observed external edit, original wording, source, sequence, supersession/withdrawal, and interpretation effects. Exercise an amendment contradicting an imported answer, direct edits of current input, repeated submission of the same amendment, and explicit correction of earlier wishes. Verify traceability survives completed/cancelled/rejected archival and ordinary later runs use current interpretation rather than reapplying historical amendments.
...
Create a small synthetic product fixture for an end-to-end demonstration: clarify a change through multiple rounds, export explained questions for a separate non-technical requestor, import and review a partially completed returned form, add an amendment changing an earlier requirement, rerun Clarify until ready, then repeat Analyze through questions/answers and amendments until a plan is generated, approve it, execute its tasks sequentially, pass the full required suite, apply the documentation increment, and archive the completed request. Also demonstrate direct implementation with an internally generated plan, blocked-work Q&A, user triage of unrelated defects, a deferred defect blocking completion through a failed required test, incremental implementation records with interrupted recovery, historical catalog lookup, and cancelled closure. Include a separate existing-product fixture for automatic first-portal initialization, reverse engineering, profile-unavailable recovery, and incremental documentation refresh. Label these fixtures as test examples, not user product requirements.
