...
AIH helps an agent understand a product, clarify requested changes, prepare plans, implement authorized work, verify results, maintain product documentation, and resume interrupted work from files.
...
Each change request has a stable identifier and its own directory. Define conventions for its request metadata, submitted input revisions and attachments, evolving definition, questionnaires, analysis, approved plan and tasks, response history, execution records, checkpoints, verification evidence, documentation impact, and closure outcome. Create artifacts when needed; avoid empty folder and template proliferation.
...
2. Clarify the ask, ask necessary questions, and preserve a refined definition.
3. Analyze and plan, including impacts, tasks, acceptance criteria, tests, and further questions when needed.
...
Clarify must not implement. Plan must not implement. Approving a plan alone must not start implementation. Offer a clearly labeled combined Approve and implement action if useful; record both actions.
...
Maintain documentation automatically as a normal workflow responsibility. Consult relevant documentation before planning and implementation. During initialization, inspect available evidence and create an appropriate baseline, recording unknowns instead of fabricating facts.
...
- `.aih/conventions/` is is the authoritative authoring source for shared framework formats, schemas, and templates.
...
Provide meaningful skills supporting clarification, planning/analysis, implementation, verification/testing, documentation maintenance, and optional Git operations. Keep shared routing in `run.md`; do not duplicate the whole orchestrator in every skill.
...
Define versioned schemas for configuration, state, requests, submissions, questionnaires, approvals, events, checkpoints, documentation nodes, evidence, and run records. Portal, helpers, and agents must use the same contracts.
...
- Change request: definition revisions, analysis, plan and approval, tasks, execution evidence, amendments, and closure actions. Browse closed requests without a backlog.
...
Provide one discoverable Python CLI, usable from the product root as `python .aih/engine/cli.py <command>`. Document the equivalent Python launcher usage on Windows and provide `--help` for every command. Implement the named operations below; select and document exact arguments consistently.
