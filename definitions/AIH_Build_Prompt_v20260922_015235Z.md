...
Prompt version: 20260922_015235Z
Generated (UTC): 2026-09-22 01:52:35Z
...
Only one change request may be open at a time. There is no backlog, queued future request, or concurrent change-request execution. Closed requests remain available as history. A separate read-only question channel is allowed.
...
Explicit Clarify, Plan, Implement, Implement directly, or Resume actions submit the latest relevant saved input and answers. Capture immutable submission revisions, attachment identities and content fingerprints, action, timestamp, and owning request. Detect repeated submissions and commands idempotently.
...
3. Analyze and plan how to satisfy the clarified requirements: evaluate solution approaches and impacts, select the design and technologies, identify affected components, and define implementation tasks and a verification approach mapped to the acceptance criteria.
...
Planning owns the implementation approach: solution design, architecture and technology choices, affected components, implementation tasks, and the verification strategy. Preserve traceability to the clarified requirements and distinguish design decisions from requirements. If planning exposes a missing requirement or consequential ambiguity, preserve the draft plan and return that question to clarification within the same request. Stop change work when the gap is consequential. Update the requirements from submitted answers, reassess affected plan approval and evidence, and resume planning from the clarified definition. Do not silently redefine requirements to suit a preferred solution.

Clarify must neither implement nor decide how the change will be implemented. Plan must not implement. Approving a plan alone must not start implementation. Offer a clearly labeled combined Approve and implement action if useful; record both actions.

The explicit Implement directly action authorizes skipping separate analysis, planning, and plan approval for its submitted scope. Record this as direct implementation authorization, not as a fictional approved plan. It does not waive necessary investigation, consequential clarification, tests, verification, documentation, evidence, or host approvals. Routine implementation reasoning and task tracking are still required.
...
Completion requires affected documentation updates or a justified no-impact determination. Documentation failures remain visible and prevent successful completion.
...
- User guides, onboarding, administration guides, support materials, known limitations, deprecation, and retirement.
...
Provide meaningful skills supporting clarification, planning/analysis, implementation, verification/testing, documentation maintenance, and optional Git operations. The clarification skill and its bundled guidance must establish what and why without choosing how; the planning skill owns implementation design and verification strategy, and can return requirement gaps to clarification. Keep shared routing in `run.md`; do not duplicate the whole orchestrator in every skill.
...
- Change request: requirements definition revisions, analysis, solution plan and approval, tasks, execution evidence, amendments, and closure actions. Distinguish requirements from design decisions, and explain that Clarify establishes what and why while Plan determines how. Browse closed requests without a backlog.
- Product documentation: progressive tree navigation, search, applicability, provenance, freshness, and unresolved reviews.
...
- Execution: Clarify, Plan, Approve plan, Implement, Implement directly, Resume, stop execution, and explicitly close as cancelled/rejected. Display sanitized live events/output through streaming or polling, identifying runs and segments, progress, process state, and final outcomes.
...
Provide one discoverable Python CLI, usable from the product root as `python .aih/engine/cli.py <command>`. Document the equivalent Python launcher usage on Windows and provide `--help` for every command, including the distinction between Clarify (what and why) and Plan (how). Implement the named operations below; select and document exact arguments consistently.
...
- Clarify, plan, approve, implement, implement directly, resume, stop execution, and close a request with an explicit outcome.
...
7. Plan approval alone does not implement; ordinary implementation requires current approval; explicit direct implementation records its authorized exception.
...
26. Clarification artifacts and questionnaires capture what and why, scope, constraints, and observable acceptance criteria without selecting an implementation. Preserve explicit user technical constraints and distinguish them from observed existing technology. Exercise read-only investigation, planning-owned design choices, and a requirement gap discovered during planning that returns to clarification. Test structural/state contracts automatically; claim the semantic phase boundary was followed only when an actual agent exercised it.
...
Create a small synthetic product fixture for an end-to-end demonstration: clarify a change, prepare and approve its plan, implement it, pass tests, update documentation, and close it; also demonstrate direct implementation, blocked-work Q&A, an amended submission, interrupted resumption, and cancelled closure. Label this fixture as a test example, not user product requirements.
